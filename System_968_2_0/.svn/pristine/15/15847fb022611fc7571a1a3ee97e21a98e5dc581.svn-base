<%@ Page Language="C#" AutoEventWireup="true" CodeFile="SystemModule_SetMealLevelPriceList.aspx.cs" Inherits="SystemModule_SetMealLevelPriceList" %>
...
<head runat="server">
    ...
    <script>
        function del(id) {
            if (confirm("确定删除此等级？")) {
                $.get("SystemModule_SetMealLevelPriceList.aspx?action=del&id=" + id, function () {
                    alert("删除成功");
        ...
        <div class="frame_width frame_top">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td style="text-align: right;">
                                <input style="float: right;" type="button" value="添加" onclick="window.location = 'SystemModule_SetMealLevelPriceEdit.aspx?setMealId=<%=setMealId%>    '" class="add_btn box_sizing box_round">
                            </td>
                        </tr>
                    </tbody>
                </table>
            ...
            <table class="lot_sub_tbl">
                <tbody>
                    ...
                    <%
                        foreach (Ac968.SystemModuleSetMeal.Model.V_SystemModuleSetMealLevel item in list)
                        { %>
                    <tr>
                        <td><%=item.Name %></td>
                        <td><%=item.Price %></td>
                        <td>
                            <a href="SystemModule_SetMealLevelPriceEdit.aspx?id=<%=item.id %>">编辑</a>
